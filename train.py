...
def train_dynamics(model, vqvae, lam, dataloader, optimizer, save_dir, epochs=EPOCHS, device="cuda"):
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, batch in enumerate(dataloader):
            ...
            # Calculate token statistics for monitoring
            n_white = (target_tokens == 1).float().sum()
            n_black = (target_tokens == 0).float().sum()
            white_ratio = (n_white / (n_white + n_black)).item()
            
            # Simple class weighting based on inverse frequency
            class_weights = torch.ones(16, device=device)
            class_weights[0] = 1.0 / (1 - white_ratio + 1e-6)  # Weight for black-associated tokens
            class_weights[1:4] = 1.0 / (white_ratio + 1e-6)    # Weight for white-associated tokens
            class_weights = class_weights / class_weights.mean()  # Normalize weights
            
            # Main prediction loss with class weights
            ...
            pred_probs = F.softmax(pred_tokens, dim=-1)
            white_mask = target_tokens == 1
            black_mask = target_tokens == 0
            
            white_loss = F.cross_entropy(
                pred_tokens[white_mask],
                target_tokens[white_mask]
            ) if white_mask.any() else torch.tensor(0.0, device=device)
            
            black_loss = F.cross_entropy(
                pred_tokens[black_mask],
                target_tokens[black_mask]
            )
            
            # Token diversity loss (from Genie paper)
            ...
            token_diversity_loss = -token_entropy
            
            # Action diversity loss (from Genie paper)
            ...
            action_diversity_loss = -action_entropy
            
            # Total loss following Genie paper
            total_loss = (
                ce_loss +                    # Main prediction loss
                0.1 * token_diversity_loss + # Token diversity (weight from paper)
                0.1 * action_diversity_loss  # Action diversity (weight from paper)
            )
            ...
            loss_tracker.update({
                ...
                'pred_diversity_loss': token_diversity_loss.item(),
            ...
            if n_batches % 10 == 0:
                ...
                print(f"  Pred Diversity Loss: {token_diversity_loss.item():.4f}")
                print(f"  InfoNCE Loss: {ce_loss.item():.4f}")
